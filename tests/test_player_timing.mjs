/* Timing-model tests for src/lib/timing.ts (docs/riso-system.md §7).
...
console.log("fixture: estimated timings, script span 15s");
{
  ...
  check(
    "last boundary ends at the audio duration",
    near(bounds[n - 1].end, 15),
  );
...
console.log("drift: the same script against a 19s track");
{
  ...
  check(
    "last slide ends WITH the audio, not before",
    near(bounds[n - 1].end, 19),
    `got ${bounds[n - 1].end}`,
  );
  check(
    "every slide still has positive duration",
  ...
  check(
    "proportions preserved (slide 1 is 2/15 of the track)",
...
console.log("no audio at all");
{
  ...
  check(
    "falls back to the script's own durations",
    near(bounds[n - 1].end, 15),
  );
...
console.log(
  "neither timings nor manifest.timings — falls back to slide durations",
);
{
  const m = structuredClone(bundle.manifest);
  delete m.timings;
  const { bounds } = buildBoundaries(
    { ...bundle, timings: null, manifest: m },
    NaN,
  );
  check(
    "derives boundaries from duration_seconds",
    near(bounds[n - 1].end, 15),
  );
  check(
    "contiguous",
