 ...
 * because the lesson flow is a tutor turn, a render POST, and the block's own
 * polling GETs. The default tutor turn does what Codex is prompted to do with
 * a named topic — answer StartLesson — and shows a probe for a gym turn.
 */
function tutorTurn(body: { state: string }): Response {
  if (body.state.startsWith("Gym turn")) return new Response(JSON.stringify(PROBE), { status: 200 });
  const match = body.state.match(/The learner wrote in the chat: (".*")\. Choose/);
  const topic = match ? (JSON.parse(match[1]) as string) : "";
  return new Response(
    JSON.stringify({ ...PROBE, componentName: "StartLesson", props: { topic } }),
    { status: 200 },
  );
}
function stubBridge(overrides: {
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url === "/api/turn") {
      return overrides.turn?.() ?? tutorTurn(JSON.parse(init?.body as string));
    }
...
describe("lesson flow", () => {
  it("starts a render for the submitted topic and shows the video block", async () => {
    ...
    // Nothing is requested until the user asks for something; the boot turn
    // is the name gate, rendered locally.
    expect(fetchMock).not.toHaveBeenCalled();
    await waitFor(() => expect(screen.getByTestId("profile-gate")).toBeDefined());
  ...
  it("plays the video and records the ready event once the job completes", async () => {
    ...
    // The tutor turn answered StartLesson, whose emit started the render…
    expect(logged[0]).toMatchObject({ component: "StartLesson", action: "topic.submitted" });
    // …and the block emits once when the video lands.
    ...
    // A finished video is not a Codex turn: the only turn was the learner's message.
    expect(callsTo(fetchMock, "/api/turn")).toHaveLength(1);
  });
...
describe("gym flow", () => {
  it("keeps /api/turn reachable behind /gym and feeds interactions back", async () => {
    ...
    expect(bodyOf(callsTo(fetchMock, "/api/turn")[0])).toMatchObject({
      state: "Gym turn. Brand new learner, nothing measured yet.",
      // turn-1 is the boot gate; the first Codex turn is the second turn.
      turnId: "turn-2",
    });
    ...
    expect(second.turnId).toBe("turn-3");
